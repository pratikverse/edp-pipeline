...
The demo shows four panels: the input drawing, the detected-component
overlay + list, the trimmed JSON (id / type / coordinates / connections
only — exactly the brief's schema, straight from emit.to_json_dict), and a
connectivity graph that the browser builds client-side from that JSON's
`connections` field, so the graph is visibly the same data as the netlist.
"""
...
_PALETTE = [
    (91, 157, 255), (159, 216, 138), (242, 179, 102), (215, 139, 240),
    (126, 193, 255), (255, 140, 90), (120, 200, 160), (240, 130, 130),
]


def _render_detection_overlay(image_path: Path, symbols: list[dict]) -> bytes:
    """Each detected symbol's box + id + type on the source image."""
    img = cv2.imread(str(image_path), cv2.IMREAD_COLOR)
    types = sorted({s["type"] for s in symbols})
    for symbol in symbols:
        x0, y0, x1, y1 = symbol["coordinates"]
        if symbol["type"] == "Unknown":
            color = (0, 0, 255)
        else:
            color = _PALETTE[types.index(symbol["type"]) % len(_PALETTE)]
        cv2.rectangle(img, (x0, y0), (x1, y1), color, 2)
        label = f"{symbol['id']} {symbol['type']}"
        (tw, th), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.4, 1)
        cv2.rectangle(img, (x0, max(0, y0 - th - 6)), (x0 + tw + 4, y0), color, -1)
        cv2.putText(img, label, (x0 + 2, max(10, y0 - 4)),
...
async def process(file: UploadFile = File(...)):
    ...
    except Exception as e:  # surface a clean error to the panel instead of a 500 page
